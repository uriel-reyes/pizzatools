...
app.get('/api/drivers', async (req, res) => {
  ...
  try {
    ...
    const drivers = response.body.results.map(customer => {
      ...
      if (customer.custom) {
        const customData = customer.custom as CustomerCustomFields;
...
app.get('/api/drivers/:id/orders', async (req, res) => {
  ...
  try {
    ...
    const driverResponse = await apiRoot
      .customers()
      .withId({ ID: driverId })
      .get()
      .execute();
    ...
    const customData = driverResponse.body.custom as CustomerCustomFields;
    let deliveryOrderRefs: any[] = [];
    ...
    for (const orderRef of deliveryOrderRefs) {
      try {
        ...
        if (order.state && (
            order.state.id === DELIVERED_STATE_ID || 
            order.state.id === CANCELLED_STATE_ID)) {
          console.log(`Skipping order ${orderId} as it is in state: ${stateInfo?.name}`);
          continue;
        }
        
        // For active drivers, only include orders that are out for delivery
        if (driverResponse.body.custom?.fields?.Dispatched === true && 
            order.state?.id !== OUT_FOR_DELIVERY_STATE_ID) {
...
app.get('/debug/recent-orders', async (req, res) => {
  try {
    ...
    const orders = response.body.results.map(order => {
      const stateInfo = order.state ? stateMap.get(order.state.id) : null;
      const customData = order.custom as any;
      const customMethod = order.custom?.fields?.Method || 
                         (customData?.customFieldsRaw?.find((f: any) => f.name === 'Method')?.value);
...
app.post('/api/dispatch', async (req, res) => {
  ...
  try {
    ...
    for (const [driverId, orderIds] of Object.entries(assignments)) {
      ...
      const updateDriverResponse = await apiRoot
        .customers()
        .withId({ ID: driverId })
        .post({
          body: {
            ...
            actions: [
              {
                ...
                fields: {
                  ...(driverResponse.body.custom?.fields || {}),
                  Dispatched: true,
...
app.post('/api/drivers/:id/return', async (req, res) => {
  ...
  try {
    ...
    const driverVersion = driverResponse.body.version;
    const deliveryOrderIds = driverResponse.body.custom?.fields?.Deliveries || [];
